...
while nor > tol && iter < iter_max
    ...
    % Instead of using the limited number of derivatives I'm gonna be lazy
    % and just take a batch of the ones we have fully computed
    
    Batch_gen = [ones(Batch_size,1);
        zeros(length(w)-Batch_size,1)];
    Batch = Batch_gen(randperm(length(w)));
    w = w - (alpha(iter+1) * g).*Batch;
    [r,J] = Res_and_Jac(w,xy);
    f = F(r);
    g = J'*r/Ntrain;
    nor = norm(g);
